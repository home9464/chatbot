...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, target, src_mask, target_mask):
        _encoded = self.encode(src, src_mask)
        _decoded = self.decode(_encoded, src_mask, target, target_mask)
        return _decoded

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, encoded, src_mask, target, target_mask):
        return self.decoder(self.target_embed(target), encoded, src_mask, target_mask)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        # forward of SublayerConnection
        x = self.sublayer[0](x, lambda p: self.self_attn(p, p, p, mask))
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, encoded, src_mask, target_mask):
        # forward of SublayerConnection
        memory = encoded
        x = self.sublayer[0](x, lambda p: self.self_attn(p, p, p, target_mask))
        x = self.sublayer[1](x, lambda p: self.src_attn(p, memory, memory, src_mask))
...
class Generator(nn.Module):
    def __init__(self, d_model, vocab):
        ...
        self.proj = nn.Linear(d_model, vocab)  # in, out
    def forward(self, x):
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    Args:
        query: Tensor[d_model, d_q] = [512, 64]
        key: Tensor[d_model, d_k] = [512, 64]
        value: Tensor[d_model, d_v] = [512, 64]
    Returns:
        pass
    """
    d_k = query.size(-1)
